/*
 * MODIFIED BY THE RE-HARMONY PROJECT, 2026.
 *
 * This file differs from upstream Concordance
 * (https://github.com/jaymzh/concordance). The change adds read-only entry
 * points that make it possible to read a Harmony One's configuration, which
 * released libconcord cannot do: its dump entry points read relative to
 * `firmware_base`, and for arch 12 that field is 0.
 *
 * The exact diff against upstream is ../libconcord-re-harmony.patch.
 * Distributed under the same licence as the code it modifies, GPL-3.0-or-later.
 */
/*
